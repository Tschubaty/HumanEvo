...
function  annotateDMRs(dm)

% anotateDMRs creates a txt file with important data about the DMRs
% --------------------------------------------------------------------
% dm = annotateDMRs(dm)
% --------------------------------------------------------------------
% Description:  antotes DMR list, adds mean methylation of other important
%               maps and write a tsv file with important information about
%               each DMR
% Input:       {dm} DMRs array.
% Output:       a txt file of DMR info
...
load (['TA_ratio_General_',dm(1).kind])
...
for chr = CHRs
    for dd = 1:dm(chr).no_DMRs
        islands{chr}(dd) = nanmax(TA_ratio_island{chr}((dm(chr).CpG_begin(dd)):(dm(chr).CpG_begin(dd))));
        if nanmax(TA_ratio_genebody{chr}((dm(chr).CpG_begin(dd)):dm(chr).CpG_begin(dd))) == 1
            genebody{chr}(dd) = 1;
        elseif nanmin(TA_ratio_genebody{chr}((dm(chr).CpG_begin(dd)):dm(chr).CpG_begin(dd))) == -1
            genebody{chr}(dd) = -1;
        else
            genebody{chr}(dd) = 0;
        end
...
DMR_counter=0;
for chr = CHRs
    for ll = 1:dm(chr).no_DMRs
        DMR_counter = DMR_counter+1;
        AllOverlappingGenes_sym = [];
        AllOverlappingPromoters_sym = [];

        idxOverlap = find(chr == UCSC{1} & ((dm(chr).gen_begin(ll) >= UCSC{2} & dm(chr).gen_begin(ll) <= UCSC{3})  |  (dm(chr).gen_end(ll)+1 >= UCSC{2} & dm(chr).gen_end(ll)+1 <= UCSC{3}) | (dm(chr).gen_begin(ll) <= UCSC{2} & dm(chr).gen_end(ll)+1 >= UCSC{3})));
        idxProms = find(chr == UCSC_Proms{1} & ((dm(chr).gen_begin(ll) >= UCSC_Proms{2} & dm(chr).gen_begin(ll) <= UCSC_Proms{3})  |  (dm(chr).gen_end(ll)+1 >= UCSC_Proms{2} & dm(chr).gen_end(ll)+1 <= UCSC_Proms{3}) | (dm(chr).gen_begin(ll) <= UCSC_Proms{2} & dm(chr).gen_end(ll)+1 >= UCSC_Proms{3})));
    ...
    for ll = 1:dm(chr).no_DMRs
        ...
        for ii = 1:length(UCSC{1})
            ...
            if UCSCchr ~= chr
                ...
                dist1 = dm(chr).gen_begin(ll) - UCSCTSS;
                dist2 = dm(chr).gen_end(ll)+1 - UCSCTSS;
                ...
                if  dm(chr).gen_begin(ll) <= UCSCTSS && dm(chr).gen_end(ll)+1 >= UCSCTSS
...
if ~isempty  (dm(chr).methylation)
    DMR_counter = 0;
    for chr = CHRs
        for dd = 1:dm(chr).no_DMRs 
            DMR_counter=DMR_counter+1;

            DMR_meth(DMR_counter,col_counter) = dm(chr).methylation(1,dd);
            DMR_meth(DMR_counter,col_counter+1) = dm(chr).methylation(2,dd);        
        end
    end
    col_counter = col_counter+2;
end

for smp = 1:length(dm(1).samples)
    load (['meth_ancient_',dm(1).kind],['TA_convperc_',dm(1).samples{smp}])
    temp = eval(['TA_convperc_',dm(1).samples{smp}]);

    DMR_counter = 0;
    for chr = CHRs
        for dd = 1:dm(chr).no_DMRs 
            DMR_counter=DMR_counter+1;
            begTA = dm(chr).CpG_begin(dd);
            finTA = dm(chr).CpG_end(dd)+1;
            DMR_meth(DMR_counter,col_counter) = nanmean(temp{chr}(begTA:finTA));

        end
    end

    clearvars(['TA_convperc_',dm(1).samples{smp}])
    col_counter = col_counter+1;
end

if strcmp (dm(1).kind,'Human')   
    %Chimp WGBS bone
...
if strcmp (dm(1).kind,'Human')
    ...
    titles_meth = [];
    regexp_meth = [];
    for tt = 1:length (dm(1).samples)
        titles_meth = strcat(titles_meth,dm(1).samples{tt}, ' meth\t');
    ...
    for chr = CHRs
        for ll = 1:dm(chr).no_DMRs
            DMR_counter = DMR_counter+1;
            fprintf(fid,reg,chr,dm(chr).gen_begin(ll),dm(chr).gen_end(ll),...
            dm(chr).max_Qt(ll),dm(chr).no_bases(ll),dm(chr).CpG_begin(ll),...
            dm(chr).CpG_end(ll), dm(chr).no_CpGs(ll),islands{chr}(ll),genebody{chr}(ll),...
