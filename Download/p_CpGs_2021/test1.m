...
sex = {'F','M','M','M','M','M','F','M','F','M','F','M','M','M','M','F','M','M',...
    'F','M','M','M','F','M','F','F','F','M','M','M','M','M','M','M','F','M',...
    'M','F','F','F','M','M','M','F','M','M','M','M','F','F','M','M','M','M',...
    'M'};
%Location = {};
...
for samp = 1:length(samples)
    ...
    eval(sprintf('load unfiltCpGs_%s',SAMPLE));
%     eval (sprintf ('load u_%s',SAMPLE))
  
    as = as.bundle(p_CpGs);
    
    if length(p_CpGs)==22
        loc_chr_names = chr_names (1:end-1);
    elseif length(p_CpGs)==23
        loc_chr_names = chr_names;  
    else
        error ('unidentified number of chromosomes')
    end
    
    as = as.set('name',sprintf('I%s',SAMPLE),'species','Homo Sapiens',...
        'chr_names',chr_names,'abbrev',SAMPLE,'library','double',...
        'reference','hg19');
...
%
% look for low and high variable regions
% W = 3000;
% chr = 9;
%{
...
% detectDMRs
dm = DMRs;
dm = dm.groupDMRs(samplist,g_names,gc,'chromosomes',chr_names,'delta',0.25);