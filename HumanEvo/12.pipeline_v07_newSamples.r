...
# doneR4 20.05.2025 Calculate state specific CpG permuation simulations dependent on delta --------------------------------------------------
if (FALSE) {
  min_delta <- 0.5
  max_q <- 0.05
  ...
  true.delta <- all_CpG_complete_with_test.45$pearson.delta[
    all_CpG_complete_with_test.45$state_group == chromatin_group & 
      all_CpG_complete_with_test.45$pearson.delta > min_delta]
  ...
  true_pvals_dt <- data.table(
    ...
    pearson.delta = all_CpG_complete_with_test.45$pearson.delta[true_filter],
