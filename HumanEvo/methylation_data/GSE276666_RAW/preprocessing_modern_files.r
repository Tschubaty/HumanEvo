...
cpg_file <- file.path(output_dir, "hg19_CpG_sites.rds")


#Check if the combined file already exists
if (file.exists(cpg_file)) {
  message("Loading existing CpG sites file...")
  cpg_df <- readRDS(cpg_file)
} else {
  ...
  for (chr in chromosomes) {
    ...
    # Save individual chromosome CpG sites as an RDS file
    saveRDS(chr_cpg_df, file = file.path(output_dir, paste0("hg19_CpG_sites_", chr, ".rds")))
  }
  
  # Combine all chromosomes into one data frame
  cpg_df <- rbindlist(cpg_list)
  
  # Save combined CpG Data for future use
  saveRDS(cpg_df, file = cpg_file)
  fwrite(cpg_df, file = file.path(output_dir, "hg19_CpG_sites.csv"))
}

message("CpG site extraction complete! Each chromosome saved separately.")


read_bismark_file <- function(file_path) {
  df <- fread(file_path, header = FALSE, sep = "\t", stringsAsFactors = FALSE)
  setnames(df, c("V1", "V2", "V3", "V4", "V5", "V6"),
           c("chrom", "start", "end", "Methylation_Percentage", "Count_Methylated", "Count_Unmethylated"))
...
sum_methylation_fast <- function(cpg_chr, sample_chr) {
  ...
  pb <- txtProgressBar(min = 0, max = nrow(cpg_chr), style = 3)
  ...
  if (length(unique_cpg_chrom) > 1 || length(unique_sample_chrom) > 1 || unique_cpg_chrom != unique_sample_chrom) {
    stop("Error: cpg_chr and sample_chr must contain data for exactly one and the same chromosome.")
  }
  ...
  for (i in seq_len(nrow(cpg_chr))) {
    ...
    while(sample_index <= n_samples && sample_chr$end[sample_index] < reference_indx_start) {
  ...
  cpg_chr[Count_Methylated == 0 & Count_Unmethylated == 0, 
          `:=`(Count_Methylated = NA, Count_Unmethylated = NA, Methylation_Percentage = NA)]
  
  # **Compute Methylation Percentage**
  cpg_chr[, Methylation_Percentage := ifelse(
    Count_Methylated + Count_Unmethylated > 0,
...
file_list <- list.files(path = this.dir, pattern = "deduplicated.bismark.cov$", full.names = TRUE)
...
for (file_path in file_list) {
  ...
  for (chr in names(cpg_list)) {
    ...
    if (chr %in% names(sample_list)) {
      ...
      merged_chr[, `:=`(Count_Methylated = NA, Count_Unmethylated = NA, Methylation_Percentage = NA)]
    }
...
smooth_methylation <- function(chrom_data, winsize) {
  ...
  if (winsize %% 2 == 0) stop("Window size must be an odd integer")
  ...
  chrom_data[, Smoothed_Count_Methylated := stats::filter(Count_Methylated, kernel, sides = 2, circular = FALSE)]
  chrom_data[, Smoothed_Total_Reads := stats::filter(Count_Methylated + Count_Unmethylated, kernel, sides = 2, circular = FALSE)]
