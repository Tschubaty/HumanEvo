...
if (ANNOTATION == "hg38") {
  ...
  H3K27ac_filename <-
    file.path(OUTPUT_FOLDER, paste("H3K27ac", "hg19", "RDS", sep = "."))
}
...
if (file.exists(H3K27ac_filename)) {
  ...
  write.table(x = H3K27ac[,1:6],
            file = file.path(OUTPUT_FOLDER, paste("H3K27ac","BED6","HG38" ,"BED", sep = ".")),
            quote = FALSE,
            row.names = FALSE,
            col.names = FALSE,sep = "\t")
  }else if(ANNOTATION == "hg19") {
    # load lifted bed hglft_genome_33500_a9ce20.bed
    H3K27ac <-
      read.delim(
        file = file.path(INPUT_FOLDER, "Lift_hg38_to_hg19", "hglft_genome_33500_a9ce20.bed"),
        header = F,
...
if (ANNOTATION == "hg38") {
  chromatin_seg_annotaion_name <- "hg38lift_genome_100_segments"
}else if(ANNOTATION == "hg19"){
  chromatin_seg_annotaion_name <- "hg19_genome_100_segments"
}
...
chromatin_seg_file_name <-
  file.path(OUTPUT_FOLDER,
            paste(chromatin_seg_annotaion_name, "RDS", sep = "."))
...
if (file.exists(chromatin_seg_file_name)) {
  ...
  chromatin_seg <-
    read.delim(
      file = file.path(INPUT_FOLDER, paste(chromatin_seg_annotaion_name, "bed", sep = ".")),
      header = F,
  ...
  for (chr in unique(chromatin_seg$chr)) {
    saveRDS(object = chromatin_seg[chromatin_seg$chr == chr, ],
            file = file.path(
              OUTPUT_FOLDER,
              paste(chr, "chromatin_seg_annotaion_name", "RDS", sep = ".")
            ))
  }
}
...
compute_chromatin_state_destribution <-
  function(target, annotation) {
    out <-
...
if(file.exists(H3K27ac_annotation_filename )){
  ...
  H3K27ac_annotation <- compute_chromatin_state_destribution(target = H3K27ac, annotation = chromatin_seg)
  saveRDS(object = H3K27ac_annotation,file = H3K27ac_annotation_filename   )
}
