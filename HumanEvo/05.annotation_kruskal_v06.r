...
meta <- openxlsx::read.xlsx(xlsxFile = "AGDP.39.metadata2.xlsx",
#  sheet,
  startRow = 1,
  colNames = TRUE,
  rowNames = FALSE,
...
df<- readRDS("C:/Users/Daniel Batyrev/Documents/GitHub/HumanEvo/HumanEvo/gatherData/real_peak_results.hg19.37.min_type_delta.0.08.rds")
...
head(df)
#df <- df[!is.na(df$best_start) & df$best_type_delta >= min_delta & df$p_val <= alpha,]
## 
library("ggplot2")
ggplot(data = df,mapping = aes(x = end-start))+ geom_histogram(breaks = seq(from = 0, to = 2000, by = 10))
...
df_sig <- df[!is.na(df$best_position) & df$best_p_val <= alpha,]

length(unique(df_sig$name))

nrow(df_sig)

bed4 <- df_sig[,c("chr","best_position","best_position","name")]

write.table(x = bed4,
            file = file.path(OUTPUT_FOLDER,paste("KS.delta",min_delta,"results","p",alpha,"bed",sep = ".")),
            quote = FALSE,row.names = FALSE,col.names = FALSE)


### make Genhance query with files https://genome.ucsc.edu/cgi-bin/hgTables

#https://genome.ucsc.edu/cgi-bin/hgTables?hgsid=1763619530_MVBTInKVQPtrGWEjRoqOlKVYwYWr&clade=mammal&org=&db=hg19&hgta_group=regulation&hgta_track=geneHancer&hgta_table=geneHancerRegElementsDoubleElite&hgta_regionType=range&position=&hgta_outputType=primaryTable&hgta_outFileName=

"KS.0.1.delta.GH-Interactions.p.0.005.bed"
df_GH <- read.delim(file =  file.path(OUTPUT_FOLDER,paste("KS",min_delta,"delta.GH-Interactions.p",alpha,"bed",sep = ".")),
                  header = FALSE,
                  skip = 1,
...
#KS.delta.0.08.results.p.0.00150343919297757.hgTables.bed
df_GH <- read.delim(file =  file.path(OUTPUT_FOLDER,"KS.delta.0.08.results.p.0.00150343919297757.hgTables.bed"),
                    header = FALSE,
                    skip = 0,
                    col.names = c(colnames(df)[1:6],"s","e","RGB"))
...
# best CpG
dir.create(file.path(OUTPUT_FOLDER,"statistic_plots"),showWarnings = FALSE)

real_sample_typisation <- readRDS("C:/Users/Daniel Batyrev/Documents/GitHub/HumanEvo/HumanEvo/04.methylation_vs_Type/real_sample_typisation.rds")

single_sample_typisation <- real_sample_typisation  
...
for(b in 1:nrow(df_sig)){
  ...
  # ggsave(filename = file.path(OUTPUT_FOLDER,"correlation plots", df_sig$name[b],paste(df_sig$annotation_narrow[b],"png",sep = ".")),plot = p,width = 4,height = 4)
    ...
    index <- !is.na(df_peak$KW_p_val) & df_peak$KW_p_val == df_sig$p_val[b]
    # choose max delta if equal p val 
    if(sum(index,na.rm = TRUE) > 1){
      index <- which(index)[which.max(df_peak$type_delta[index])]
    }
    
    single_sample_typisation$meth <- as.numeric(df_peak[index,single_sample_typisation$sample])

    scatter_plot <- ggplot(data = single_sample_typisation ,mapping =  aes(x = type, y = meth,color = type)) + 
      geom_boxplot()+
      geom_point(position = "jitter") +
      ggtitle(paste(df_peak$chrom[index],df_peak$start[index],"p_val:",format.pval(df_peak$KW_p_val[index])),paste("delta mean", df_peak$type_delta[index]))+
      theme_minimal()+
      ylim(0,1)

    ggsave(filename =file.path(OUTPUT_FOLDER,"statistic_plots",paste(df_sig$annotation_narrow[b] ,"png",sep = ".")),plot = scatter_plot)
}

write.table(x = df_sig,
            file = file.path(OUTPUT_FOLDER,paste("Ks_type_test","delta",min_delta,"annotaed_peak_with_best_CpG_results","p",alpha,"bed",sep = ".")),
            quote = FALSE,row.names = FALSE,col.names = FALSE)
...
write.csv2(
  ...
  file = file.path(
    OUTPUT_FOLDER,
    paste(
      "Ks_type_test","delta",min_delta,"annotaed_peak_with_best_CpG_results","p",alpha,
      "txt",
      sep = "."
    )
  ),
