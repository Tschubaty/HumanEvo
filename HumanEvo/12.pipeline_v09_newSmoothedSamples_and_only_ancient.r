...
##
##  output: pipeline
##  # histogramm matched updat: all_CpG.45.samples.histogram.merged2026.hg19.rds
##  
##  v_09 21.12.2025
...
OUTPUT_FOLDER <- "12.pipeline_2026"
...
if(FALSE){
  ...
  load_variable_if_not_exists(
    ...
    file_path = "methylation+chip/all_CpG.45.samples.histogram.merged2026.hg19.rds"
  )
  ...
  all_CpG.meth <-
    all_CpG.45.samples.hist.hg19[, as.character(meta$sample),with = FALSE]
  ...
  for (n_chr in seq_along(CHR_NAMES)) {
    message("chr: ", CHR_NAMES[n_chr])
    file_name <- file.path(OUTPUT_FOLDER, "results", "H3K27ac",
                           paste(CHR_NAMES[n_chr], "CpG_annotation", "rds", sep = "."))
    if (file.exists(file_name)) next
    
    # select columns once (fast)
    df_chr <- all_CpG.45.samples.hist.hg19[chrom == CHR_NAMES[n_chr], 1:6]
    setDT(df_chr)
    
    # load annotation
    chr_chromatin_seg <- readRDS(file.path("Chromatin/processed",
                                           paste(CHR_NAMES[n_chr], "chromatin_seg_annotaion_name", "RDS", sep = ".")))
    setDT(chr_chromatin_seg)
    setorderv(chr_chromatin_seg, "end")
    
    # Vectorized mapping (fast)
    idx <- findInterval(df_chr$end, chr_chromatin_seg$end)
    df_chr[, state := ifelse(idx == 0L, NA_character_, chr_chromatin_seg$state[idx])]
    
    saveRDS(df_chr, file_name)
