...
OUTPUT_FOLDER <- "07.methylation_p_val"
N_SAMPLES <- 39
ANNOTATION <- "hg19"
...
for (chr in CHR_NAMES) {
  ...
  #time function over chromosome 
  start_time <- Sys.time()
  
  df_chr <-
    readRDS(file = file.path(
      INPUT_FOLDER,
      paste(chr, ANNOTATION, N_SAMPLES, "samples", "rds", sep = ".")
    ))
  df_chr_meth <- df_chr[, as.character(levels(meta$sample))]
  
  # score stores number of data points
  df_chr$score  <-
    parRapply(
      cl = clus,
      x = df_chr_meth,
  ...
  # delta = difference between max and min recorded score
  df_chr$delta_meth <-
    parRapply(
      cl = clus,
      x = df_chr_meth,
