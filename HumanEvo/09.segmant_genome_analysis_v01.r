...
df_all_CpG <-
  readRDS(
    "C:/Users/Daniel Batyrev/Documents/GitHub/HumanEvo/HumanEvo/07.methylation_p_val/all.hg19.39.samples.p_val.delta_meth.rds"
  )
df_peak_CpG <- df_all_CpG[df_all_CpG$name != "NO_CHIP", ]
...
df_peaks <-
  foreach(peak = unique(df_peak_CpG$name), .combine = rbind) %dopar% {
    ...
    n_CpG <- sum(indices)
    delta_meth <- df_peak_CpG$delta_meth[indices]
    p_val <- df_peak_CpG$p_val[indices]
    # relevant CpGs
    index_CpG_relevant <-
      delta_meth >= min_delta & !is.na(p_val)
    ...
    if (sum(index_CpG_relevant) > 0) {
      ...
      pearson_cor <- df_peak_CpG$pearson_cor[indices]
