...
# set running paramteres ----------------------------------------------------------
create_permutations <- FALSE


# load Data ---------------------------------------------------------------

df_peak_CpG <- readRDS("C:/Users/Daniel Batyrev/Documents/GitHub/HumanEvo/HumanEvo/methylation+chip/H3K27ac.only.39.samples.merged.hg19.rds")
...
parallel_testing_kruskall_valis <- function(df,typisation){
  ...
  stopCluster(clust)
  gc()
  df_x <- data.frame(kw.p_val = p_val,kw.statistic = statistic,kw.delta = delta)
  return(df_x)
}
